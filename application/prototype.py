...
ROAD_TYPE_FIELD = "fclass"  # change if needed
...
roads_gdf_full = load_roads(ROADS_SHP_PATH)

# Keep type column if it exists
if ROAD_TYPE_FIELD in roads_gdf_full.columns:
    roads_gdf = roads_gdf_full[[ROAD_TYPE_FIELD, "geometry"]].copy()
else:
    roads_gdf = roads_gdf_full[["geometry"]].copy()
...
# Tabs
# -----------------------------
tab1, tab2 = st.tabs(["Road Quality (Features)", "Road Types (OSM)"])
...
with tab1:
    ...
    # Sentinel-2 RGB composite for context
    add_ee_layer(m, S2_COMPOSITE, {"bands": ["B4", "B3", "B2"], "min": 0, "max": 3000}, "Sentinel-2 RGB")

    folium.LayerControl().add_to(m)

    st.subheader("Map")
    map_event = st_folium(m, width=1200, height=600, key="map_tab1")
...
# =============================
# TAB 2: Road Types (checkboxes)
# =============================
with tab2:
    st.subheader("OSM Road Types")

    if ROAD_TYPE_FIELD not in roads_gdf_full.columns:
        st.warning(f"Column `{ROAD_TYPE_FIELD}` not found. Please set ROAD_TYPE_FIELD.")
    else:
        type_values = sorted(roads_gdf_full[ROAD_TYPE_FIELD].dropna().unique().tolist())

        st.write("Select road types to display:")
        selected_types = []
        cols = st.columns(3)
        ...
        roads_types = roads_gdf_full[roads_gdf_full[ROAD_TYPE_FIELD].isin(selected_types)][[ROAD_TYPE_FIELD, "geometry"]].copy()
        roads_types = roads_types.to_crs("EPSG:4326")
        roads_types["geometry"] = roads_types["geometry"].simplify(SIMPLIFY_TOL, preserve_topology=True)

        colors = [
            "#FF3B30", "#FF9500", "#FFCC00", "#34C759", "#007AFF",
            "#5856D6", "#AF52DE", "#FF2D55", "#8E8E93", "#00C7BE", "#A2845E"
        ]
        color_map = {t: colors[i % len(colors)] for i, t in enumerate(type_values)}

        m2 = folium.Map(location=[7.95, -1.02], zoom_start=7, tiles=None)
        folium.TileLayer(
            tiles="https://mt1.google.com/vt/lyrs=s&x={x}&y={y}&z={z}",
            attr="Google",
            name="Google Satellite",
        ...
        for t in selected_types:
            gdf_t = roads_types[roads_types[ROAD_TYPE_FIELD] == t]
            if not gdf_t.empty:
                folium.GeoJson(
                    gdf_t,
                    name=str(t),
                    style_function=lambda x, col=color_map[t]: {"color": col, "weight": 1}
                ).add_to(m2)

        folium.LayerControl().add_to(m2)
        st_folium(m2, width=1200, height=600, key="map_tab2")
