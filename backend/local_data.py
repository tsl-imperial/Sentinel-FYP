...
Data sources:
  ...
  data/layer1_full_network_predictions.parquet      Layer 1 paved/unpaved predictions
...
LAYER1_PREDICTIONS_PARQUET = DATA_DIR / "layer1_full_network_predictions.parquet"
...
@functools.lru_cache(maxsize=1)
def _load_layer1_predictions() -> pd.DataFrame:
    """Road-level Layer 1 paved/unpaved model predictions, indexed by osm_id.

    The file is generated by notebooks/layer1_full_network_prediction.ipynb.
    Keep this loader non-fatal when the file is absent so the app can still
    run with just the Sentinel-2 index parquet.
    """
    cols = [
        "osm_id",
        "fclass",
    ...
    if not LAYER1_PREDICTIONS_PARQUET.exists():
        return pd.DataFrame(columns=cols).set_index("osm_id")
    df = pd.read_parquet(LAYER1_PREDICTIONS_PARQUET)
    for col in cols:
        if col not in df.columns:
            df[col] = pd.NA
    df = df[cols].copy()
    df["osm_id"] = df["osm_id"].astype(str)
    return df.set_index("osm_id").sort_index()
...
def layer1_prediction_for_osm_id(osm_id: str) -> dict[str, Any] | None:
    """Layer 1 paved/unpaved prediction for a single OSM road id."""
    df = _load_layer1_predictions()
    if df.empty:
        return None
    try:
        row = df.loc[str(osm_id)]
    except KeyError:
        return None
    if isinstance(row, pd.DataFrame):
        row = row.iloc[0]
    ...
    def _bool_or_none(value: Any) -> bool | None:
        return bool(value) if value is not None and pd.notna(value) else None

    return {
        "pred_surface": str(row["pred_surface"]) if pd.notna(row.get("pred_surface")) else None,
        "prob_paved": _float_or_none(row.get("prob_paved")),
        "prob_unpaved": _float_or_none(row.get("prob_unpaved")),
        "confidence": _float_or_none(row.get("confidence")),
        "low_confidence": _bool_or_none(row.get("low_confidence")),
        "is_labelled_train_road": _bool_or_none(row.get("is_labelled_train_road")),
    }
...
def region_summaries() -> tuple:
    ...
    Returns a tuple of dicts:
        ({"name", "road_km", "edge_count", "area_km2",
          "class_composition", "surface_composition", "surface_count"}, ...)
    ...
    predictions = _load_layer1_predictions()
    if not predictions.empty:
        pred_joined = joined[["osm_id", "region", "length_m"]].merge(
            predictions[["pred_surface", "confidence", "low_confidence"]],
            left_on="osm_id",
            right_index=True,
            how="inner",
        )
        pred_joined = pred_joined[pred_joined["pred_surface"].isin(["paved", "unpaved"])]
        by_region_surface_km = (
            pred_joined.groupby(["region", "pred_surface"], sort=False)["length_m"].sum() / 1000.0
        )
        by_region_surface_count = pred_joined.groupby(["region", "pred_surface"], sort=False).size()
    else:
        by_region_surface_km = pd.Series(dtype="float64")
        by_region_surface_count = pd.Series(dtype="int64")
    ...
    for region in sorted(by_region.index):
        ...
        surface_composition = {
            surface: float(by_region_surface_km.get((region, surface), 0.0))
            for surface in ("paved", "unpaved")
        }
        surface_count = {
            surface: int(by_region_surface_count.get((region, surface), 0))
            for surface in ("paved", "unpaved")
        }
        summaries.append({
            "name": region,
